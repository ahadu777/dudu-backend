...
#### 1.1 购买流程 (Story A - Purchase)

- [x] **TC-PROD-001**: 浏览商品目录
  - 操作: 打开小程序/网页 → 进入商品列表页
  - **Expected**: 显示所有可购买商品，包含名称、价格、库存状态
  - **Result**: ✅ 通过 (2025-12-20)

- [x] **TC-PROD-002**: 查看商品详情
  - 操作: 点击商品卡片 → 进入详情页
  - **Expected**: 显示商品权益（ferry/bus/mrt）、价格、可用日期
  - **Result**: ✅ 通过 (2025-12-20)

- [x] **TC-ORDER-001**: 创建订单
  - 操作: 选择商品 → 填写联系人信息 → 点击"提交订单"
  - **Expected**: 订单创建成功，显示待支付状态和 15 分钟倒计时
  - **Result**: ✅ 通过 (2025-12-20)

#### 1.2 支付流程 (Story B - Payment)

- [x] **TC-PAY-001**: 完成支付
  - 操作: 在订单页点击"去支付" → 完成微信支付
  - **Expected**: 支付成功，订单状态变为"已支付"，收到票券
  - **Result**: ✅ 通过 (2025-12-20)

#### 1.3 票券查看 (Story C - View & QR)

- [x] **TC-TICKET-001**: 查看我的票券
  - 操作: 进入"我的票券"页面
  - **Expected**: 显示已购买的票券列表，包含权益信息（ferry/bus/mrt 各 X 次）
  - **Result**: ✅ 通过 (2025-12-20)

- [x] **TC-QR-001**: 生成核销二维码
  - 操作: 点击票券 → 点击"出示二维码"
  - **Expected**: 显示动态二维码，有效期 60 秒，支持刷新
  - **Result**: ✅ 通过 (2025-12-20)

#### 1.4 核销流程 (Story D - Redemption)

- [x] **TC-OP-001**: 操作员登录
  - 操作: 打开验票系统 → 输入用户名密码 → 点击登录
  - **Expected**: 登录成功，进入扫码页面
  - **Result**: ✅ 通过 (2025-12-20)

- [x] **TC-VERIFY-001**: 扫码核销
  - 操作: 点击"扫一扫" → 扫描用户二维码 → 选择核销的权益 → 点击确认
  - **Expected**: 核销成功，显示"核销成功"，用户权益使用次数减 1
  - **Result**: ✅ 通过 (2025-12-20)
...
- [x] **TC-ORDER-002**: 库存不足时下单
  - 操作: 选择库存为 0 的商品 → 尝试下单
  - **Expected**: 提示"库存不足"，无法创建订单
  - **Result**: ✅ 通过 (2025-12-20)

- [x] **TC-ORDER-003**: 订单超时未支付
  - 操作: 创建订单 → 等待 15 分钟 → 查看订单状态
  - **Expected**: 订单自动取消，状态变为"已取消"
  - **Result**: ✅ 通过 (2025-12-20)
...
- [x] **TC-PAY-002**: 取消支付
  - 操作: 在支付界面点击"取消"
  - **Expected**: 返回订单页，订单状态保持"待支付"
  - **Result**: ✅ 通过 (2025-12-20)

- [ ] **TC-PAY-003**: 重复支付通知（幂等性）
  - 操作: 同一订单发送两次支付成功通知
  - **Expected**: 订单状态正确，不会重复出票
  - **Note**: ⬜ 需后端配合测试

#### 2.3 核销异常

- [x] **TC-VERIFY-002**: 重复核销同一权益
  - 操作: 已核销的票券 → 再次扫码尝试核销
  - **Expected**: 提示"该权益已使用"或"次数已用完"
  - **Result**: ✅ 通过 (2025-12-20)

- [x] **TC-VERIFY-003**: 二维码过期后扫描
  - 操作: 等待二维码过期（>60秒）→ 扫描过期二维码
  - **Expected**: 提示"二维码已过期，请刷新"
  - **Result**: ✅ 通过 (2025-12-20)
...
| 版本 | 日期 | 作者 | 变更内容 |
|------|------|------|----------|
| 2.1 | 2025-12-18 | AI | 添加 QA E2E Checklist（14 scenarios） |
