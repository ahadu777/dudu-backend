...
- [x] **TC-OTA-101**: OTA 合作伙伴认证
  - 操作: OTA 使用授权密钥登录系统
  - **Expected**: 认证通过，可访问 OTA 专属功能
  - **Result**: ✅ 通过 (2025-12-20)

- [x] **TC-OTA-102**: 查询可用库存
  - 操作: OTA 查询当前可售产品库存
  - **Expected**: 显示产品 106-108 的可用库存，总计约 5000 套餐
  - **Result**: ✅ 通过 (2025-12-20)

- [x] **TC-OTA-103**: 创建批量预订
  - 操作: OTA 提交 25 份预订请求
  - **Expected**: 预订成功，库存减少 25
  - **Result**: ✅ 通过 (2025-12-20)

- [x] **TC-OTA-104**: 批量生成预售票券
  - 操作: OTA 批量生成 10 张票券
  - **Expected**: 生成 10 张待激活状态票券，每张有唯一编码
  - **Result**: ✅ 通过 (2025-12-20)

- [x] **TC-OTA-105**: 为客户激活票券
  - 操作: OTA 填写客户信息并激活票券
  - **Expected**: 票券状态变为已激活，可出示二维码
  - **Result**: ✅ 通过 (2025-12-20)

- [x] **TC-OTA-106**: 查询客户订单
  - 操作: OTA 查询指定订单的票券信息
  - **Expected**: 显示订单票券列表，含二维码和客户信息
  - **Result**: ✅ 通过 (2025-12-20)

- [ ] **TC-OTA-107**: 客户在场馆核销
  - 操作: 客户到场 → 操作员扫描二维码
  - **Expected**: 核销成功，权益减少，流程与直销票券一致
  - **Note**: ⏳ 待测试 - OTA 票券核销流程验证

- [x] **TC-OTA-108**: 渠道库存隔离验证
  - 操作: 对比直销渠道和 OTA 渠道的库存数量
  - **Expected**: OTA 和直销库存独立，无冲突
  - **Result**: ✅ 通过 (2025-12-20)

- [x] **TC-OTA-109**: 查询票券列表
  - 操作: OTA 按状态筛选查询票券列表
  - **Expected**: 显示分页票券列表，仅显示该 OTA 的票券
  - **Result**: ✅ 通过 (2025-12-20)

- [ ] **TC-OTA-110**: 票券使用完毕状态更新
  - 操作: 核销完所有权益
  - **Expected**: 票券状态自动更新为已使用
  - **Note**: ⏳ 待测试 - 依赖 TC-OTA-107

### Round 2: 异常场景 (6 scenarios)

- [x] **TC-OTA-201**: 无授权密钥访问
  - 操作: 不提供授权密钥直接访问 OTA 功能
  - **Expected**: 系统拒绝访问，提示需要授权
  - **Result**: ✅ 通过 (2025-12-20)

- [x] **TC-OTA-202**: 无效授权密钥
  - 操作: 使用错误的授权密钥尝试登录
  - **Expected**: 系统拒绝访问，提示密钥无效
  - **Result**: ✅ 通过 (2025-12-20)

- [x] **TC-OTA-203**: 超出单次预订限制
  - 操作: 尝试预订 150 份（超过单次限制 100）
  - **Expected**: 系统拒绝，提示单次预订限制 1-100
  - **Result**: ✅ 通过 (2025-12-20)

- [x] **TC-OTA-204**: 预订不存在产品
  - 操作: 尝试预订一个不存在的产品
  - **Expected**: 系统拒绝，提示产品不存在
  - **Result**: ✅ 通过 (2025-12-20)

- [x] **TC-OTA-205**: 超出批量生成限制
  - 操作: 尝试批量生成 150 张票券（超过限制）
  - **Expected**: 系统拒绝，提示数量超限
  - **Result**: ✅ 通过 (2025-12-20)

- [x] **TC-OTA-206**: 激活不存在的票券
  - 操作: 使用无效票券编码尝试激活
  - **Expected**: 系统拒绝，提示票券不存在
  - **Result**: ✅ 通过 (2025-12-20)

### Round 3: 边界测试 (4 scenarios)

- [ ] **TC-OTA-301**: 大批量预订
  - 操作: OTA 连续创建多个 100 份预订
  - **Expected**: 库存正确扣减，无并发冲突
  - **Note**: ⏳ 待测试 - 高并发场景验证

- [x] **TC-OTA-302**: 批量生成票券上限
  - 操作: 批量生成 100 张票券（上限）
  - **Expected**: 成功生成，所有票券状态正确
  - **Result**: ✅ 通过 (2025-12-20)

- [x] **TC-OTA-303**: 防重复核销跨终端
  - 操作: 同一二维码在不同场馆扫描
  - **Expected**: 第二次扫描提示已核销
  - **Result**: ✅ 通过 (2025-12-20)
...
| 版本 | 日期 | 作者 | 变更内容 |
|------|------|------|----------|
| 1.2 | 2025-12-20 | Claude | 更新测试结果 16/20 通过，优化描述为业务语言 |
