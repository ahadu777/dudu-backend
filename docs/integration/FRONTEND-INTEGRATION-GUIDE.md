...
- Instance URL: `https://dudu-derp-cxk5g.ondigitalocean.app/`
- Mode: Currently using **mock data** (USE_DATABASE=false)
- Production: Switch to `USE_DATABASE=true` to use Directus
...
**Frontend Logic:**
```typescript
async function checkTicketEligibility(ticketCode: string): Promise<boolean> {
  try {
...
async function createReservation(
  ...
  const isEligible = await checkTicketEligibility(ticketCode);
...
const ticketCode = 'TKT-ACTIVE-001';
...
const ticketCode = 'TKT-ACTIVE-001'; // From QR code
...
curl -X POST http://localhost:8080/api/tickets/validate \
  ...
  -d '{"ticket_code":"TKT-ACTIVE-001","orq":1}'
...
curl -X POST http://localhost:8080/operators/validate-ticket \
  -H "Content-Type: application/json" \
  -d '{"ticket_code":"TKT-ACTIVE-001","operator_id":"OP-001","terminal_id":"TERM-A","orq":1}'
```
